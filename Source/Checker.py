...
def file_input():
    cop_file = pickle.loads(input())
    spy_file = pickle.loads(input())
    
    return cop_file,spy_file
    ...
    while True:
        if steps <=max_steps:
            if control:
                ...
                temp = copy(cops.pos)
                timing.temp = temp
                output,_time_=timing.time_limit('spy_handle.info(temp)')
